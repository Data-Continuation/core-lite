...
def source_scope(event: Dict[str, Any]) -> str:
    ...
    if isinstance(payload, dict) and payload.get("scope"):
    ...
    if isinstance(identity_hashes, dict):
        for name in ["bundle_hash", "manifest_hash", "state_hash", "fingerprint_hash"]:
            if identity_hashes.get(name):
                return f"{name}:{identity_hashes[name]}"
        ...
        if index == 0:
            if actual not in {None, ""}:
                missing_links.append({
                    ...
                    "reason": "first event should not require previous_local_event_hash",
                    "actual": actual,
                })
            ...
            missing_links.append({
                "index": index,
                "event_hash": event.get("event_hash"),
                "reason": "previous_local_event_hash mismatch",
...
def validate_parent_links(events: List[Dict[str, Any]]) -> Dict[str, Any]:
    event_hashes = {event.get("event_hash") for event in events if event.get("event_hash")}
    missing_parents: List[Dict[str, Any]] = []

    for event in events:
        parent = event.get("parent_event_hash")
        if parent and parent not in event_hashes:
    ...
    return {
        "valid_locally": not missing_parents,
...
def build_latest_validated_sources(events: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for index, event in enumerate(events):
        ...
        status = {
            ...
            "index": index,
            "validated": event_is_validated(event),
            "confirmation_status": event.get("confirmation_status"),
        }

        if event_is_validated(event):
            latest[scope] = status
        elif event.get("outcome") in TERMINAL_BLOCKING_OUTCOMES:
            latest[scope] = {**status, "blocking": True}
...
def can_allow_for_scope(index: Dict[str, Any], scope: str) -> Dict[str, Any]:
    ...
    if latest is None:
        return {"scope": scope, "allow": False, "reason": "no validated source for scope"}

    if latest.get("blocking"):
        return {"scope": scope, "allow": False, "reason": f"latest source is blocking outcome {latest.get('outcome')}", "latest": latest}

    if not local_chain.get("valid"):
        return {"scope": scope, "allow": False, "reason": "local chain invalid", "latest": latest}

    if not parent_chain.get("valid_locally"):
        return {"scope": scope, "allow": False, "reason": "parent chain has unresolved local parents", "latest": latest}
...
def generate_source_state_index(repo_root: Path, repository: str = "unknown/unknown") -> Dict[str, Any]:
    ...
    index = {
        "schema": "stegverse_source_state_index.v1",
        "generated_at": utc_now(),
        "repository": repository,
        "event_count": len(events),
        "latest_validated_sources": latest,
        "chain_status": {
            "local_chain": validate_local_chain(events),
            "parent_links": validate_parent_links(events),
        },
        "allow_status": {},
    }

    for scope in sorted(latest):
        index["allow_status"][scope] = can_allow_for_scope(index, scope)
