...
def _remote_repo_from_git(repo_root: Path) -> Optional[str]:
    try:
        completed = subprocess.run(["git", "config", "--get", "remote.origin.url"], cwd=repo_root, capture_output=True, text=True, check=False)
...
def detect_context(repo_root: Path) -> Dict[str, str]:
    ...
    org, repo = repository.split("/", 1) if "/" in repository else ("unknown", repository)
    return {
