...
def all_bundle_files(staging_root: Path) -> List[str]:
    return sorted([p.relative_to(staging_root).as_posix() for p in staging_root.rglob("*") if p.is_file()])

def validate_manifest(manifest: Dict[str, Any], context: Dict[str, str], staging_root: Path, repo_root: Path | None = None) -> Dict[str, Any]:
    ...
    unclassified = [p for p in bundle_files if p not in declared and p not in allowed_special and not is_excluded(p, excluded)]
    ...
    if repo_root is not None:
        ...
        report = repo_root / ".stegverse" / "manifest_admissibility_report.json"
        report.parent.mkdir(parents=True, exist_ok=True)
        report.write_text(json.dumps(admissibility, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        if not admissibility.get("success"):
            raise ValueError(f"manifest admissibility failed: {admissibility.get('errors', [])}")
    return {"target_repo": target, "current_repo": current, "declared_count": len(declared), "bundle_file_count": len(bundle_files), "unclassified": unclassified, "excluded_patterns": excluded, "manifest_admissibility": admissibility}
