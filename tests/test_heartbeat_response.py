...
class HeartbeatResponseTests(unittest.TestCase):
    def setUp(self):
        ...
        self.message={"message_id":"msg-00000001","exchange_id":"ex-00000001","source_org":"StegVerse-Labs","destination_org":"Data-Continuation","stage":"SENT","detail_class":"AWARENESS","retention_class":"PROJECT","authority":self.authority}
        self.config={"organization":"Data-Continuation","supported_detail_classes":["MEMORY","ACTION","AWARENESS","AUTHORITY","EVIDENCE","BLOCKER","CAPABILITY","CONTEXT"],"response_detail_class":"CAPABILITY"}
    def test_select(self): self.assertEqual(select_message({"messages":[self.message]},"Data-Continuation")["message_id"],"msg-00000001")
    def test_authority_escalation_rejected(self):
        m=dict(self.message); m["authority"]=dict(self.authority,execution=True)
        with self.assertRaises(ValueError): select_message({"messages":[m]},"Data-Continuation")
    ...
    def test_missing_fails_closed(self):
        with self.assertRaises(ValueError): select_message({"messages":[]},"Data-Continuation")
